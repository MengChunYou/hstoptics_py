...
# -*- coding: utf-8 -*-
"""
ST-DBSCAN: A spatio-temporal clustering algorithm.

Original source: https://github.com/eubr-bigsea/py-st-dbscan
Author: eubr-bigsea contributors
License: (Check original repo for license details)

Time Handling: Since the third dimension is arbitrary in this project, we treat center_point[2] as a numerical value and calculate the min_time and max_time by simply subtracting and adding the temporal_threshold.
"""
import pandas as pd
...
class STDBSCAN(object):
    ...
    def _retrieve_neighbors(self, index_center, matrix):
        ...
        # Original code: Assuming `center_point[2]` is a datetime object and using timedelta for time manipulation
        ...
        # New code: Treating `center_point[2]` as a numerical value (e.g., seconds) and directly performing the operation
        min_time = center_point[2] - self.temporal_threshold
        max_time = center_point[2] + self.temporal_threshold
        
        # Filter the matrix by time range (min_time, max_time)
        ...
        neigborhood = matrix[tmp <= (
            self.spatial_threshold*self.spatial_threshold), 4].astype(int).tolist()
